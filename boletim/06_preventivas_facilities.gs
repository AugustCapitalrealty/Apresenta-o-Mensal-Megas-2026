/**
 * ARQUIVO: 06_preventivas_facilities.gs
 * Slide de Manutenção Preventiva — versão FACILITIES.
 *
 * Diferenças vs. gerarSlide06_Preventivas():
 *   - Aba: "megas QUADRO COMPARATIVO"
 *   - Linha 39 = agendadas, 45 = realizadas, 23 = datas
 *   - Datas ajustadas -1 dia (dia 01 → último dia do mês anterior)
 *   - Cards SLA: apenas Geral, Facilities e Property (sem Operação Hangar)
 */

function gerarSlide06_Preventivas_Facilities() {
  ...
  // --- 0. EXTRAÇÃO DE DADOS ---
  // =========================================================
  Logger.log("Extraindo dados Preventivas Facilities...");

  const SHEET_NAME = 'megas QUADRO COMPARATIVO';
  const ROW_AGEND  = 39;
  const ROW_REAL   = 45;
  const ROW_DATE   = 23;

  let slaFac = "N/D", slaProp = "N/D", slaGeral = "N/D";
  ...
  try {
    ...
    // --- SLA por área (mesmas células, nova aba) ---
    const getVal = (cell) => {
    ...
    // Últimas 8 colunas que têm valor em Agendadas
    let colIndexes = [];
    ...
    colIndexes.forEach(i => {
      ...
      // Rótulo da data com ajuste -1 dia
      ...
      if (rowDate[i]) {
        ...
        if (dateObj) {
          // Regra de negócio: subtrai 1 dia (ex.: dia 01 → último do mês anterior)
    ...
    Logger.log("Erro Preventivas Facilities: " + e.message);
  ...
  // =========================================================
  // --- 2. CARDS DE SLA — apenas Geral, Facilities, Property ---
  // =========================================================
  ...
  const slaItems = [
    ...
    { title: 'SLA FACILITIES',   val: slaFac,   col: CR_DESIGN_SYSTEM.colors.brandSoft,                         main: false },
    { title: 'SLA PROPRIEDADES', val: slaProp,  col: CR_DESIGN_SYSTEM.colors.brandMed,  main: false },
  ];
