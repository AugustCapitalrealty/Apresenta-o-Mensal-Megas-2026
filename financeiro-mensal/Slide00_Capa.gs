...
function gerarSlideCapa() {
  ...
  _dsAplicarMarcaEscura_(slide, W, H,
    'CAPITAL REALTY · FINANCEIRO', 'Expandir Eficiência');
  ...
  over.getText().setText(_capaEspacado_('Grupo Capital Realty')).getTextStyle()
    .setFontSize(9).setBold(true).setForegroundColor(DS.colors.highlight).setFontFamily(DS.typography.titles);

  // Barra de destaque em gradiente acima do título
  _capaGradiente_(slide, 46, H * 0.30 + 26, 66, 4, DS.colors.brandLight, DS.colors.highlight, { steps: 12 });
  ...
  grupo.getText().setText('Resultados Financeiros').getTextStyle()
    .setFontSize(24).setBold(true).setForegroundColor(DS.colors.highlight).setFontFamily(DS.typography.titles);
  ...
  pillBorda.getBorder().getLineFill().setSolidFill(DS.colors.highlight, 0.35); pillBorda.getBorder().setWeight(1);
