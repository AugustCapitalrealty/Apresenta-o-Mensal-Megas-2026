...
function _fmVacanciaDesenhar_(c, rows) {
  ...
  rows.forEach((r, i) => {
    const y = top + i * rh;
    _rrUmaLinha_(c.slide, c.m, y, chartW * .38, rh, r.nome,
      { fs: c.W * .0105, fsMin: c.W * .0105, bold: true,
        fonte: c.DS.typography.body, align: 'L' });
    const bg = c.slide.insertShape(SlidesApp.ShapeType.RECTANGLE, c.m + chartW * .39, y + rh * .25, chartW * .46, rh * .5); bg.getFill().setSolidFill(c.DS.colors.lines); bg.getBorder().setTransparent();
    const fg = c.slide.insertShape(SlidesApp.ShapeType.RECTANGLE, c.m + chartW * .39, y + rh * .25, Math.max(1, chartW * .46 * Math.min(1, r.v)), rh * .5); fg.getFill().setSolidFill(c.DS.colors.brandLight); fg.getBorder().setTransparent();
    _rrUmaLinha_(c.slide, c.m + chartW * .86, y, chartW * .14, rh, r.vacancia,
      { fs: c.W * .0105, fsMin: c.W * .0105, bold: true,
  ...
  heads.forEach((h, i) => { _rrCelula_(c.slide, xx, top, ws[i], c.H * .09, c.DS.colors.tableHeader); _rrBloco_(c.slide, xx, top, ws[i], c.H * .09, _rrFormatarCabecalhoTabela_(h), { fs: fsHeader, fsMin: fsHeader, bold: true, cor: '#FFF', fonte: c.DS.typography.titles, folga: _RR_RECUO_TEXTBOX / 2 }); xx += ws[i]; });
  rows.forEach((r, ri) => { xx = x; [r.nome, r.area, r.ocupada, r.disponivel].forEach((v, i) => { _rrCelula_(c.slide, xx, top + c.H * .09 + ri * rh, ws[i], rh, ri % 2 ? c.DS.colors.tableStripe : null); _rrUmaLinha_(c.slide, xx + (i ? 0 : 3), top + c.H * .09 + ri * rh, ws[i] - (i ? 0 : 3), rh, v, { fs: fsBody, fsMin: fsBody, bold: i === 0, fonte: c.DS.typography.body, align: i ? 'C' : 'L' }); xx += ws[i]; }); });
}
